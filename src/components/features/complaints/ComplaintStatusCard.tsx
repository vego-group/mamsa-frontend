 ...
 * them. When one stops loading the card offers a reload, which re-reads the
 * complaint — expected behaviour, not a fault.
 */
...
export function ComplaintStatusCard({
  ...
  useEffect(() => setBrokenImages(0), [complaint]);
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
            {complaint.images.map((img, i) => (
              // Keyed on the url so a refreshed link remounts the <img> and retries.
              <a
                key={`${i}-${img.url}`}
                href={img.url}
                target="_blank"
              ...
              >
                <img
                  ...
                  onError={() => setBrokenImages((n) => n + 1)}
                />
              </a>
            ))}
          </div>
          {brokenImages > 0 && (
            <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-brand-surface p-3 text-xs text-brand-muted">
              <span>{t('imagesExpired')}</span>
