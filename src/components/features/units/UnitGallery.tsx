  ...
  return (
    <>
      ...
      <div className="relative mb-8">
        ...
        >
          <img src={main.card} alt={title} className="h-full w-full object-cover transition group-hover:scale-105" />
          <span
            dir="ltr"
        ...
        <div className="hidden h-[440px] grid-cols-4 grid-rows-2 gap-2 md:grid">
          ...
          {thumbs.map((img, i) => (
            ...
            >
              <img src={img.card} alt={`${title} ${i + 2}`} className="h-full w-full object-cover transition duration-300 group-hover:scale-105" />
              <span className="absolute inset-0 bg-black/0 transition group-hover:bg-black/10" />
            </button>
          ))}
        </div>
      ...
      {lightbox !== null && current && (
        <div className="fixed inset-0 z-[80] flex h-[100dvh] flex-col bg-black/95" dir="rtl">
          ...
          {/* thumbnail strip */}
          {count > 1 && (
            <div className="flex shrink-0 gap-2 overflow-x-auto px-4 py-4">
              {images.map((img, i) => (
                <button
                  key={i}
